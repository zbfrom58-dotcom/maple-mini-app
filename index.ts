...
// URL мини-приложения, который отправляется кнопкой в /start.
// Задайте APP_URL в переменных окружения Railway (например https://your-app.up.railway.app)
...
function auth(req: any): string {
  ...
  if (!tgId) {
    const error: any = new Error('Открой приложение через Telegram');
...
// Webhook Telegram: команды/сообщения + подтверждение платежей
app.post('/telegram/webhook', async (req: any, reply) => {
  try {
    const update = req.body || {};

    // Пользователь написал боту /start (или открыл его впервые)
    const messageText = update.message?.text;
    if (typeof messageText === 'string' && messageText.startsWith('/start')) {
      const chatId = update.message.chat.id;
      await sendTelegramMessage(
        chatId,
        '🍁 Привет! Лови листики, выполняй задания и приглашай друзей.'
      );
      return { ok: true };
    }
